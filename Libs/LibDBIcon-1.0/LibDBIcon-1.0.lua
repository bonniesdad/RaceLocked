...
local function createButton(name, object, db, customCompartmentIcon)
  ...
  if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE then
    ...
    bg:SetPoint('CENTER', button, 'CENTER')
...
function lib:Lock(name)
  local b = lib:GetMinimapButton(name)
  if b then
...
function lib:Show(name)
  ...
  if b then
    b:Show()
    updatePosition(b, b.db and b.db.minimapPos or b.minimapPos)
  end
end
...
function lib:Refresh(name, db)
  ...
  if b then
    ...
    updatePosition(b, b.db and b.db.minimapPos or b.minimapPos)
    if not b.db or not b.db.hide then
      b:Show()
    else
      b:Hide()
    end
    if not b.db or not b.db.lock then
      b:SetScript('OnDragStart', onDragStart)
      b:SetScript('OnDragStop', onDragStop)
    else
      b:SetScript('OnDragStart', nil)
...
function lib:IsButtonCompartmentAvailable()
  return AddonCompartmentFrame and true
end
function lib:IsButtonInCompartment(buttonName)
  local o = lib.objects[buttonName]
  return o and o.db and o.db.showInCompartment
end
...
for name, button in next, lib.objects do
  ...
  if not button.fadeOut then
    button.fadeOut = button:CreateAnimationGroup()
    local a = button.fadeOut:CreateAnimation('Alpha')
    a:SetOrder(1)
    a:SetDuration(0.2)
    a:SetFromAlpha(1)
    a:SetToAlpha(0)
