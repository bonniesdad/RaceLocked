...
function CallbackHandler.New(_self, target, RegisterName, UnregisterName, UnregisterAllName)
  ...
  target[RegisterName] = function(self, eventname, method, ...)
    if type(eventname) ~= 'string' then
      error('Usage: eventname - string expected.', 2)
    end
    ...
    if type(method) == 'string' then
      if type(self) ~= 'table' or self == target then
        error('Usage: bad self', 2)
      end
      if type(self[method]) ~= 'function' then
        error('method not found on self', 2)
      end
      regfunc = select('#', ...) >= 1 and function(...)
        self[method](self, select(1, ...), ...)
      end or function(...)
        self[method](self, ...)
      end
    else
